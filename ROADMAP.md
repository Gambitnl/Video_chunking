...
- [x] **Refine snippet placeholder output**
  ...
  - Status: [DONE] Completed 2025-10-26 (Codex GPT-5)
- [ ] **GPU Memory Exhaustion During Pipeline Processing**
  - Files: `src/pipeline.py`, `src/transcription.py`, `src/diarization.py`, `src/classification.py`
  - Issue: GPU memory not properly released between pipeline stages, causing cumulative exhaustion
  - Symptoms:
    - CUDA "unknown error" during speaker embedding extraction (Stage 5 - Diarization)
    - Ollama "memory layout cannot be allocated" errors (Stage 6+ - Classification)
    - GGML_ASSERT failures in Ollama after prolonged processing
    - Successful transcription (Stage 3) followed by diarization failures
  - Root Cause: Long transcription sessions (~33 min for 3.7hr audio) leave GPU memory allocated, preventing subsequent stages from loading models
  - Observed in: Session test_nov10_2020pm (2025-11-10) - CUDA errors for SPEAKER_04-18, classification failures for segments 2-5, 15, 23, 55, 74
  - Fix Strategy:
    1. Add explicit GPU memory cleanup between pipeline stages (`torch.cuda.empty_cache()`, `gc.collect()`)
    2. Implement CUDA health checks before each GPU-intensive stage
    3. Add graceful recovery from CUDA errors (detect corruption, attempt reset)
    4. Configure Ollama memory limits to prevent overallocation
    5. Consider stage isolation via subprocess spawning for critical GPU stages
    6. Add memory usage monitoring and logging throughout pipeline
  - Estimated effort: 2-3 days
  - Impact: CRITICAL - prevents pipeline failures on long sessions, improves reliability
  - Priority: P0 (blocks successful processing of 3+ hour sessions)
  - Status: [TODO] Not Started (Identified 2025-11-10)
- [ ] **PyAnnote Version Compatibility Warnings**
  - Files: `src/diarization.py`, `requirements.txt`
  - Issue: Version mismatches between trained models and runtime environment
  - Warnings:
    - "Model was trained with pyannote.audio 0.0.1, yours is 3.1.1"
    - "Model was trained with torch 1.8.1+cu102, yours is 2.5.1+cu121"
    - "Bad things might happen unless you revert..."
  - Impact: Potential diarization accuracy degradation, increased likelihood of CUDA errors
  - Fix Strategy:
    1. Research recommended pyannote.audio + PyTorch version combinations
    2. Test diarization accuracy across version combinations
    3. Either: (a) pin to compatible versions, or (b) upgrade models to match current versions
    4. Document version requirements and upgrade path in README
    5. Add version compatibility checks in diarization module
  - Estimated effort: 1-2 days (research + testing)
  - Impact: HIGH - improves diarization stability and accuracy
  - Priority: P0 (contributes to GPU stability issues)
  - Status: [TODO] Not Started (Identified 2025-11-10)
- [ ] **Ollama Model Memory Configuration**
  - Files: `src/classification.py`, `src/config.py`, `.env.example`
  - Issue: gpt-oss:20b model runs out of memory during classification stage
  - Symptoms: "memory layout cannot be allocated (status code: 500)" even with low_vram retry
  - Root Cause: No explicit memory limits configured for Ollama, 20B model too large after GPU exhaustion
  - Fix Strategy:
    1. Add Ollama memory limit configuration options (OLLAMA_MAX_MEMORY, OLLAMA_GPU_LAYERS)
    2. Implement automatic model size detection and memory requirement estimation
    3. Add fallback to smaller models when memory allocation fails (20b -> 13b -> 7b)
    4. Consider offloading classification to CPU or external service for large models
    5. Add pre-stage memory availability checks before loading Ollama models
    6. Document recommended model sizes for different GPU memory configurations
  - Estimated effort: 1-2 days
  - Impact: HIGH - enables classification to complete successfully on resource-constrained systems
  - Priority: P0 (prevents classification completion)
  - Related: GPU Memory Exhaustion issue above
  - Status: [TODO] Not Started (Identified 2025-11-10)
