...
- [~] BUG-20251119-102: SessionAnalyzer.calculate_character_stats relies on a non-existent `duration` field in transcript segments, so any caller receives zero speaking durations even though `start_time`/`end_time` are provided. Update the helper to compute duration from timestamps and add regression tests. (Agent: Claude Sonnet 4.5, Started: 2025-11-19 14:00 UTC) -> src/analytics/session_analyzer.py:373-394
