...
#### `src/classifier.py` - Duplicated Response Parsing
- **Lines 109-193:** `BaseClassifier._parse_response()` method
- **Lines 578-604:** Similar parsing in `GroqClassifier.classify_segments()`
- **Issue:** Response parsing logic duplicated
- **Suggestion:** Use template method pattern consistently
...
### `src/classifier.py` - Multiple Functions
- **Lines 71-78:** `_build_prompt()` - 5 parameters
- **Lines 58-63:** `classify_segments()` - 3 parameters
- **Suggestion:** Create `ClassificationContext` object

### `src/knowledge_base.py:88` - `extract_knowledge()` - 3 parameters
- **Note:** Acceptable but could use context object
...
- **`src/formatter.py:220`** - Preview length → `PREVIEW_TEXT_MAX_LENGTH = 220`
...
**Primary Concerns:**
- ❌ pipeline.py is too large and complex (2,334 lines, 600+ line method)
- ❌ Significant code duplication in transcriber backends
- ❌ Inconsistent error handling patterns
- ❌ Many magic numbers not extracted to constants
- ❌ Long parameter lists without configuration objects

### Impact Analysis

**Lines of Code Reduction:**
- Eliminating duplication: ~250 lines
- Extracting repeated patterns: ~150 lines
- Better abstractions: ~300 lines
- **Total: ~700 lines removed (12% reduction)**
...
**Total Issues Identified:** 78 refactoring candidates across 12 categories
